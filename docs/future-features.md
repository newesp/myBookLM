...
### 3. Contradiction 開新對話討論（next up）

**目標**：lint 找到的 contradiction 由使用者透過 chat 討論釐清。

**流程**
1. lint 結果頁的 contradiction 列加「💬 與 AI 討論」按鈕
2. Backend `POST /api/wiki/repair/discuss` body `{issue}`
   - 建立新 conversation，topic_id 帶入或 null
   - 預先注入 system prompt：「你正在協助使用者釐清以下矛盾」
   - 預先注入 user message：「請對比 page A 與 page B 在 X 點的陳述，提出 3 個可能的化解方案」
   - 把兩頁全文塞進 wiki context
3. Frontend 自動切到 chat 分頁、自動切到該對話
4. **精簡版**：使用者多輪討論完，覺得 OK 就手動 `📖 存入 Wiki`（複用既有 ingest 路徑）
5. **進階版**（待實作）：偵測 conversation 有 `repair_metadata`，在對話下方多顯示「結論動作」面板（改頁 A / 改頁 B / 兩頁都加 disambiguation / 建 compare 頁 / 已釐清不需改）
