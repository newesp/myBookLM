...
- **2026-05-07 — Source-to-Wiki ingest**
  - `wiki._source_to_chunks(source, slug)` — skill.md 一章一 chunk（過長章節按段落切，cap `SOURCE_INGEST_CHUNK_BUDGET=12000` chars）；embedding-only 把連續 DB chunks 合併成 ~budget 大小的群組
  - `wiki._format_source_chunk(...)` — 把 slug / name / segment / 建議 citation / 內文組成 `_ingest` 用的 `new_content`
  - `wiki.count_source_chunks(...)` — 不花 token 的 pre-flight，給前端 confirm dialog 用
  - `wiki.ingest_source(wiki_dir, slug, source, cfg)` — 對每個 chunk 跑一輪 Plan + Apply；最後在 log.md 寫一行總計 (`creates / updates / tokens`)
  - `GET /api/wiki/ingest/source/preview?slug=...` → `{slug, name, types, chunk_count}`
  - `POST /api/wiki/ingest/source` body `{slug}` → 完整摘要 (`chunks_total`, `creates`, `updates`, per-chunk breakdown)
  - 前端 source ⋮ 選單加 `📖 灌入 Wiki`：先呼叫 preview 取得 chunk 數，confirm 顯示「預估 N–N×4 次 LLM 呼叫」並建議切便宜 model
- **2026-05-07 — Step 4: Duplicate diff 預覽 + 套用**
  - 新 prompt `repair-duplicate-merge.md` — LLM 挑 primary / secondary，產生合併後內容 + redirect stub
  - `wiki.plan_repair_duplicate(wiki_dir, issue, cfg)` — 純計畫，不寫檔；回傳 `plan_id` (`_REPAIR_PLAN_CACHE`，process-local，5 分鐘 TTL)
  - `wiki.apply_repair_plan(wiki_dir, plan_id)` — 套用前把每頁舊內容寫進 `log.md`（`--- BEFORE <path> ---` 區塊），per-wiki lock 防 race；plan 套用後即從 cache 移除
  - `POST /api/wiki/repair/plan` body `{issue}` (僅 `category=duplicate`) → `{plan_id, primary, secondary, reasoning, actions:[{path, role, before, after}], expires_at, ttl_seconds, tokens_in, tokens_out}`
  - `POST /api/wiki/repair/apply` body `{plan_id}` → `{ok, plan_id, applied}`
  - 前端 lint 結果 duplicate 列加 🛠 LLM 修按鈕（紫色 `.lint-plan-btn`）
  - 新 modal `#repair-modal` 顯示左右兩欄 diff（自手刻 LCS line-diff，紅綠對齊）
  - 三顆按鈕：✅ 套用 / 🔄 重新生成 / ❌ 取消
  - 套用成功後 `refreshWikiAfterFix({category: "duplicate", page})` 把 lint 列從 DOM 拔掉
  - 進階版（✏️ 編輯後套用 / textarea 編輯 proposed content）留給未來
